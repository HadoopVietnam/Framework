 ...
 */
package com.hadoopvietnam.cache.memcached;
...
public class MemcachedCache implements Cache {
     ...
     * Constructor.
     *
     * @param inClient the memcached client to set.
     * @param inMaxListSize the maximum size a list can grow to before it is
     * trimmed
     */
    ...
    /**
     * @param inMaxListSize the maxListSize to set
     */
     ...
     * {@inheritDoc}
     */
    public void set(final String inKey, final int timeExpiration, final Object inValue) {
        if (log.isTraceEnabled()) {
            log.trace("Setting to cache '" + inKey + "' - " + inValue);
        }

        if (inValue != null) {
            client.set(inKey, timeExpiration, inValue);
        } else {
            // a null really should be a delete of the key
            log.warn("null passed in as parameter for key " + inKey
                    + ".  Deleting key from memcached to force client reload.");
